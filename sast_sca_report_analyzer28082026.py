...
def fetch_sast_results_from_api(risk_score_threshold=None, project_limit=None, project_name_filter=None):
    """
    ...
    if project_name_filter:
        sast_projects = [p for p in sast_projects if p.get('name', '') == project_name_filter]
        if not sast_projects:
            print(f"No SAST project found with name: {project_name_filter}")
            return []
        print(f"Filtering to SAST project: {project_name_filter}")
...
def SCA_get_project_latest_scan_id(project_name, access_token=""):
    """Get scan ID and date for a project from the project lookup response.

    Uses latestScanId, falling back to lastSuccessfulScanId. Does not call
    `/risk-management/scans/{scanId}` (some environments return 403).
    ...
    try:
        ...
        scan_date = _sca_project_scan_date(project_obj)
...
def extract_scan_date_from_zip(zip_ref):
    """Read the actual scan date from RiskReportSummary.csv inside the export zip."""
    summary_file = 'RiskReportSummary.csv'
    if summary_file not in zip_ref.namelist():
        return None

    try:
        summary_df = pd.read_csv(zip_ref.open(summary_file))
        if summary_df.empty:
            return None

        row = summary_df.iloc[0]
        for column in ('CreatedOn', 'ScanCompletedOn', 'ScanDate', 'LastScanDate'):
            if column in summary_df.columns and pd.notna(row[column]):
                value = row[column]
                if str(value).strip():
                    return value
    except Exception as e:
        print(f"Warning: Could not read scan date from {summary_file}: {e}")

    return None

def _sca_project_scan_date(project_obj):
    """Best-effort scan date from project metadata (may be missing)."""
    if not isinstance(project_obj, dict):
        return None
    return (
        project_obj.get('lastScanDate') or
        project_obj.get('lastScanTime') or
        project_obj.get('scanDate')
    )
    ...
    try:
        ...
        try:
            with zipfile.ZipFile(report_zip_path, 'r') as zip_ref:
                ...
                zip_scan_date = extract_scan_date_from_zip(zip_ref)
                effective_scan_date = zip_scan_date or scan_date
                last_scanned = format_date_to_dd_mm_yyyy(effective_scan_date) if effective_scan_date else 'N/A'
                if zip_scan_date:
                    print(f"Scan date from report: {last_scanned}")
                elif effective_scan_date:
                    print(f"Scan date from project metadata: {last_scanned}")
...
def export_to_excel_with_summary(all_results_data, risk_summary_data, output_file, risk_score_threshold=None, color_report=False, project_limit=None, project_name_filter=None):
    ...
    try:
        ...
        sast_results = fetch_sast_results_from_api(risk_score_threshold, project_limit=project_limit, project_name_filter=project_name_filter)
...
def SCA_bulk_download_reports(projects, access_token):
    ...
    def resolve_scan(project):
        ...
        scan_date = _sca_project_scan_date(project)

        if latest_id:
            return project, latest_id, scan_date

        if successful_id:
            return project, successful_id, scan_date

        print(f"  No scan ID for: {name}")
