...
API_TIMEOUT = 30  # seconds timeout for each API call
...
#########################################
SCA_account = os.getenv('SCA_ACCOUNT', '')
SCA_username = os.getenv('SCA_USERNAME', '')
SCA_password = os.getenv('SCA_PASSWORD', '')

SCA_proxy = os.getenv('SCA_PROXY', '')
...
SAST_username = os.getenv('SAST_USERNAME', '')
SAST_password = os.getenv('SAST_PASSWORD', '')
SAST_server_name = os.getenv('SAST_SERVER_NAME', '')

# SAST Proxy (separate from SCA proxy)
SAST_proxy = os.getenv('SAST_PROXY', '')  # Leave empty if no proxy needed
...
def SCA_get_access_token():
    ...
    try:
        ...
        response = requests.post(SCA_auth_url, headers=headers, data=payload, verify=False, proxies=proxy_servers, timeout=API_TIMEOUT)
...
def SAST_get_access_token():
    ...
    try:
        ...
        response = requests.post(SAST_auth_url, headers=headers, data=payload, verify=False, proxies=SAST_proxy_servers, timeout=API_TIMEOUT)
        response.raise_for_status()
        access_token = response.json()['access_token']
...
def SAST_get_all_projects(access_token=""):
    ...
    try:
        ...
        response = requests.get(url, headers=headers, proxies=SAST_proxy_servers, verify=False, timeout=API_TIMEOUT)
        response.raise_for_status()
...
def SAST_get_project_last_scan_id(project_id, access_token=""):
    ...
    Returns: tuple (scan_id, scan_date) or (None, None) if no scan found
    """
    if not access_token:
        access_token = SAST_get_access_token()
    ...
    try:
        ...
        response = requests.get(url, headers=headers, params=params, proxies=SAST_proxy_servers, verify=False, timeout=API_TIMEOUT)
        response.raise_for_status()
        scans = response.json()
        ...
        if isinstance(scans, list) and len(scans) > 0:
            scan = scans[0]
            scan_id = scan.get('id')
            # Try different possible field names for scan date
            scan_date = (
                scan.get('dateAndTime') or
                scan.get('scanDate') or
                scan.get('finishedOn') or
                scan.get('startedOn') or
                scan.get('createdAt')
            )
            return scan_id, scan_date
...
def SAST_download_scan_report(scan_id, report_type='CSV', access_token=""):
    ...
    try:
        ...
        response = requests.post(url, headers=headers, json=payload, proxies=SAST_proxy_servers, verify=False, timeout=API_TIMEOUT)
        response.raise_for_status()
        ...
        while attempt < max_attempts:
            status_response = requests.get(status_url, headers=headers, proxies=SAST_proxy_servers, verify=False, timeout=API_TIMEOUT)
            ...
            if status == 'Created':
                ...
                download_response = requests.get(download_url, headers=headers, proxies=SAST_proxy_servers, verify=False, timeout=API_TIMEOUT)
...
def parse_sast_csv_report(csv_content):
    ...
    Returns: Dictionary with (vuln_type, severity) -> {'count': count, 'detection_date': detection_date}
    """
    try:
        ...
        # Find Detection Date column
        for col in df.columns:
            if 'detection' in col.lower() and 'date' in col.lower():
        ...
        if detection_date_col:
            print(f"  Using columns: Query='{query_col}', Severity='{severity_col}', DetectionDate='{detection_date_col}'")
        else:
            print(f"  Using columns: Query='{query_col}', Severity='{severity_col}'")
            print(f"  Warning: Detection Date column not found in CSV")
        ...
        for _, row in df.iterrows():
            ...
            # Get detection date if available and format it
            detection_date = 'N/A'
            if detection_date_col and pd.notna(row[detection_date_col]):
                detection_date = format_date_to_dd_mm_yyyy(row[detection_date_col])

            key = (vuln_type, severity)

            if key not in vulnerability_summary:
                vulnerability_summary[key] = {'count': 0, 'detection_date': detection_date}

            vulnerability_summary[key]['count'] += 1
            # Keep the first detection date found for this vulnerability type
            if vulnerability_summary[key]['detection_date'] == 'N/A' and detection_date != 'N/A':
                vulnerability_summary[key]['detection_date'] = detection_date
...
def fetch_sast_results_from_api(risk_score_threshold=None, project_limit=None):
    """
    ...
    if project_limit and project_limit > 0:
        sast_projects = sast_projects[:project_limit]
        print(f"Project limit applied: analyzing first {len(sast_projects)} SAST project(s)")
    ...
    for i, project in enumerate(sast_projects, 1):
        ...
        try:
            ...
            # Get last scan ID and scan date
            scan_id, scan_date = SAST_get_project_last_scan_id(project_id, sast_token)
            if not scan_id:
            ...
            last_scanned = format_date_to_dd_mm_yyyy(scan_date) if scan_date else 'N/A'
            safe_print(f"  Latest scan ID: {scan_id}, Scan Date: {last_scanned}")
            ...
            for (vuln_type, severity), vuln_data in vulnerability_summary.items():
                # Handle both old format (int count) and new format (dict with count and detection_date)
                if isinstance(vuln_data, dict):
                    count = vuln_data.get('count', 0)
                    detection_date = vuln_data.get('detection_date', 'N/A')
                else:
                ...
                all_sast_results.append({
                    ...
                    'count': count,
                    'LastScanned': detection_date  # SAST Detection Date from CSV
                })
                vuln_count += count
                vuln_type_count += 1
                safe_print(f"    - {severity}: {vuln_type} (count: {count}, detection date: {detection_date})")
...
def SCA_get_all_projects(access_token=""):
    ...
    try:
        ...
        response = requests.get(url, headers=headers, proxies=proxy_servers, verify=False, timeout=API_TIMEOUT)
        response.raise_for_status()
...
def SCA_get_scan_status(scan_id, headers):
    """Return the status string of a single scan, or None on error."""
    try:
        scan_url = SCA_api_url + f"/risk-management/scans/{scan_id}"
        r = requests.get(scan_url, headers=headers, proxies=proxy_servers, verify=False, timeout=API_TIMEOUT)
        r.raise_for_status()
        data = r.json()
        return data
    except Exception as e:
        print(f"Warning: Could not fetch scan details for scan ID {scan_id}: {str(e)}")
        return None

def _sca_status_name(scan_data):
    """Extract status string whether status field is a str or nested dict like {'name': 'Failed'}."""
    status = scan_data.get('status', '') if isinstance(scan_data, dict) else ''
    if isinstance(status, dict):
        return status.get('name', '')
    return str(status)

def _sca_scan_date(scan_data):
    """Extract the best available scan date from a scan data dict."""
    if not isinstance(scan_data, dict):
        return None
    return (
        scan_data.get('scanCompletedOn') or
        scan_data.get('scanStartedOn') or
        scan_data.get('createdOn') or
        scan_data.get('updatedOn') or
        scan_data.get('scanDate') or
        scan_data.get('lastScanDate')
    )

SCA_SUCCESSFUL_STATUSES = {'Done', 'Completed', 'Succeeded', 'Success'}

def SCA_get_project_latest_scan_id(project_name, access_token=""):
    """Get the latest *successful* scan ID and date for a project.

    1. Try latestScanId — if its status is successful, use it.
    2. Fall back to lastSuccessfulScanId already present in the project object
       (no extra API call needed).

    Note:
    Some environments forbid calling `/risk-management/scans/{scanId}` (403).
    To stay compatible, this function will *not* require scan-detail calls.

    Returns: tuple (scan_id, scan_date) or ("", None) if nothing usable found.
    """
    if not access_token:
        access_token = SCA_get_access_token()
    ...
    try:
        ...
        # Endpoint may return a list or a single dict depending on API version
        if isinstance(response_json, list):
            if not response_json:
                print(f"No project found with name: {project_name}")
                return "", None
            project_obj = response_json[0]
        elif isinstance(response_json, dict):
            project_obj = response_json
        else:
            print(f"Unexpected response format for project lookup: {type(response_json)}")
            return "", None

        latest_scan_id = project_obj.get('latestScanId', '')
        last_successful_id = project_obj.get('lastSuccessfulScanId', '')

        # Prefer a scan id from the project lookup response without calling scan-detail APIs.
        scan_date = (
            project_obj.get('lastScanDate') or
            project_obj.get('lastScanTime') or
            project_obj.get('scanDate') or
            project_obj.get('createdOn') or
            project_obj.get('updatedOn')
        )

        if latest_scan_id:
            print(f"Using latestScanId {latest_scan_id} for '{project_name}' (scan date: {scan_date})")
            return latest_scan_id, scan_date

        if last_successful_id:
            print(f"Using lastSuccessfulScanId {last_successful_id} for '{project_name}' (scan date: {scan_date})")
            return last_successful_id, scan_date

        print(f"No scan ID found for project: {project_name}")
        return "", None
...
def SCA_get_report(project_name, report_type='csv', access_token=""):
    ...
    Returns: tuple (report_path, scan_date) or ("", None) if failed
    """
    if not access_token:
        access_token = SCA_get_access_token()

    scan_id, scan_date = SCA_get_project_latest_scan_id(project_name, access_token)
    if scan_id:
        try:
            ...
            response = requests.post(url, headers=headers, json=payload, proxies=proxy_servers, verify=False, timeout=API_TIMEOUT)
            if not response.ok:
                print(f"Export request failed [{response.status_code}]: {response.text}")
            response.raise_for_status()
            ...
            while attempt < max_attempts:
                status_response = requests.get(status_url, headers={'Authorization': 'Bearer ' + access_token}, proxies=proxy_servers, verify=False, timeout=API_TIMEOUT)
                status_response.raise_for_status()
                status_data = status_response.json()
                ...
                if export_status == 'Completed':
                    ...
                    if file_url:
                        ...
                        download_response = requests.get(file_url, proxies=proxy_servers, verify=False, timeout=API_TIMEOUT)
                        download_response.raise_for_status()
...
def format_date_to_dd_mm_yyyy(date_value):
    ...
    Handles:
    - ISO format: 2025-06-18T14:07:40.000Z -> 18-06-2025
    - US format: 2/29/2024 12:13:55 PM -> 29-02-2024
    - ISO with timezone: 2026-01-21T21:58:51.236Z -> 21-01-2026
    - Already formatted dates
    - N/A and empty values

    Returns: String in DD-MM-YYYY format or 'N/A'
    """
    if date_value is None or pd.isna(date_value) or str(date_value).upper() in ['N/A', 'NAN', '']:
        return 'N/A'

    try:
        date_str = str(date_value).strip()

        # If already N/A, return as is
        if date_str.upper() == 'N/A':
            return 'N/A'

        # Try to parse the date using pandas (handles most formats)
        ...
        # If parsing fails, return the original value or N/A
        return 'N/A'

def analyze_report_from_zip(report_zip_path, project_name, show_all_severities=False, show_transitive=True, risk_score_threshold=None, show_dev_dependencies=True, team=None, scan_date=None):
        ...
        try:
            ...
            if csv_filename in extracted_data:
                ...
                for _, row in packages_df.iterrows():
                    ...
                    # Apply risk score threshold filter, but skip it for restricted licenses
                    parsed_risk_score = parse_risk_score(risk_score)
                    if risk_score_threshold is not None and parsed_risk_score is not None and not license_risk:
                    ...
                    if severity not in ['none', 'info']:
                        if show_all_severities or severity == 'high':
                            ...
                            # Get release dates and format them
                            release_date = format_date_to_dd_mm_yyyy(row['ReleaseDate']) if has_release_date and pd.notna(row['ReleaseDate']) else 'N/A'
                            newest_version = row['NewestVersion'] if has_newest_version and pd.notna(row['NewestVersion']) else 'N/A'
                            newest_release_date = 'N/A'
                            if has_newest_release_date and pd.notna(row[newest_date_column]):
                                newest_release_date = format_date_to_dd_mm_yyyy(row[newest_date_column])
                            ...
                            results_data.append({
                                ...
                                'LastScanned': last_scanned  # SCA scan date
                            })
                    ...
                    if license_risk:
                        ...
                        if severity in ['none', 'info']:
                            # Get release dates for license-only issues and format them
                            release_date = format_date_to_dd_mm_yyyy(row['ReleaseDate']) if has_release_date and pd.notna(row['ReleaseDate']) else 'N/A'
                            newest_version = row['NewestVersion'] if has_newest_version and pd.notna(row['NewestVersion']) else 'N/A'
                            newest_release_date = 'N/A'
                            if has_newest_release_date and pd.notna(row[newest_date_column]):
...
def export_to_excel_with_summary(all_results_data, risk_summary_data, output_file, risk_score_threshold=None, color_report=False, project_limit=None):
    ...
    try:
        ...
        # Fetch SAST results from API
        sast_results = fetch_sast_results_from_api(risk_score_threshold, project_limit=project_limit)
...
def SCA_bulk_download_reports(projects, access_token):
    """Submit export requests for all projects in parallel, poll together,
    download all ZIPs, and return a list of dicts:
        {project_name, project_obj, report_path, scan_date}

    Failed/skipped projects are silently omitted from the result list.
    """
    ...
    # ── Step 1: resolve the best scan ID for every project concurrently ──────
    print(f"\nResolving scan IDs for {len(projects)} projects...")

    def resolve_scan(project):
        name = project.get('name', '')
        latest_id     = project.get('latestScanId', '')
        successful_id = project.get('lastSuccessfulScanId', '')
        hdrs = {'Authorization': 'Bearer ' + access_token}

        def try_scan(sid):
            data = SCA_get_scan_status(sid, hdrs)
            if data and _sca_status_name(data) in SCA_SUCCESSFUL_STATUSES:
                return sid, _sca_scan_date(data)
            return None, None

        if latest_id:
            sid, date = try_scan(latest_id)
            if sid:
                return project, sid, date

        if successful_id and successful_id != latest_id:
            sid, date = try_scan(successful_id)
            if sid:
                return project, sid, date

        print(f"  No successful scan for: {name}")
        return project, None, None

    resolved = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as ex:
        futures = {ex.submit(resolve_scan, p): p for p in projects}
        for f in concurrent.futures.as_completed(futures):
            project, scan_id, scan_date = f.result()
            if scan_id:
                resolved.append((project, scan_id, scan_date))

    print(f"Resolved {len(resolved)}/{len(projects)} projects with valid scans")

    if not resolved:
        return []

    # ── Step 2: submit all export requests ───────────────────────────────────
    print(f"\nSubmitting {len(resolved)} export requests...")
    export_url = SCA_api_url + "/export/requests"

    pending = []   # list of (project, scan_id, scan_date, export_id)
    for project, scan_id, scan_date in resolved:
        name = project.get('name', '')
        try:
            payload = {'scanId': scan_id, 'fileFormat': 'ScanReportCsv'}
            r = requests.post(export_url, headers=headers, json=payload,
                              proxies=proxy_servers, verify=False, timeout=API_TIMEOUT)
            if not r.ok:
                print(f"  Export request failed for {name} [{r.status_code}]: {r.text}")
                continue
            export_id = r.json().get('exportId')
            if export_id:
                pending.append((project, scan_id, scan_date, export_id))
            else:
                print(f"  No exportId returned for {name}")
        except Exception as e:
            print(f"  Exception submitting export for {name}: {e}")

    print(f"Submitted {len(pending)} export requests, polling for completion...")

    # ── Step 3: poll all exports together ────────────────────────────────────
    poll_headers = {'Authorization': 'Bearer ' + access_token}
    completed  = {}   # export_id -> file_url
    failed_ids = set()
    max_polls  = 60   # up to 2 minutes total
    poll       = 0

    while pending and poll < max_polls:
        time.sleep(2)
        poll += 1
        still_pending = []
        for item in pending:
            project, scan_id, scan_date, export_id = item
            name = project.get('name', '')
            try:
                status_url = SCA_api_url + f"/export/requests?exportId={export_id}"
                sr = requests.get(status_url, headers=poll_headers,
                                  proxies=proxy_servers, verify=False, timeout=API_TIMEOUT)
                sr.raise_for_status()
                data = sr.json()
                ...
                if status == 'Completed':
                    ...
                    if file_url:
                        completed[export_id] = (project, scan_id, scan_date, file_url)
                    else:
                        print(f"  No fileUrl for {name}")
                elif status == 'Failed':
                    print(f"  Export failed for {name}")
                    failed_ids.add(export_id)
                else:
        ...
        if pending:
            names = [p.get('name','?') for p,_,_,_ in pending]
            print(f"  Poll {poll}: {len(completed)} done, {len(pending)} pending: {names[:5]}{'...' if len(names)>5 else ''}")

    if pending:
        print(f"  Timed out waiting for {len(pending)} exports")

    print(f"\nDownloading {len(completed)} completed reports...")

    # ── Step 4: download all ZIPs ────────────────────────────────────────────
    results = []
    for export_id, (project, scan_id, scan_date, file_url) in completed.items():
        name = project.get('name', '')
        try:
            dr = requests.get(file_url, proxies=proxy_servers, verify=False, timeout=API_TIMEOUT)
            dr.raise_for_status()
            report_path = os.path.join(os.getcwd(), name + '_SCA_report.zip')
            with open(report_path, 'wb') as f:
                f.write(dr.content)
            results.append({
                'project':      project,
    ...
    print(f"Downloaded {len(results)}/{len(projects)} reports successfully")
    return results


def analyze_all_projects(show_all_severities=False, show_transitive=True, keep_report=False, risk_score_threshold=None, show_dev_dependencies=True, output_csv=None, delay_between_projects=0.1, color_report=False, email_recipients=None, fast=False, project_limit=None):
    """Analyze all projects in the SCA portal.

    Default mode is sequential/reliable. Use fast=True to enable bulk parallel download.
    """
    try:
        ...
        if project_limit and project_limit > 0:
            ...
            print(f"Project limit applied: analyzing first {len(projects)} project(s)")
        ...
        if fast:
            print("Fast mode enabled: bulk parallel download")
            downloaded = SCA_bulk_download_reports(projects, access_token)
            if not downloaded:
                print("No reports downloaded")
                return 1

            failed_projects = len(projects) - len(downloaded)
            for i, item in enumerate(downloaded, 1):
                project_name = item['project_name']
                report_file = item['report_path']
                sca_scan_date = item['scan_date']
                project_obj = item['project']

                project_team = None
                assigned_teams = project_obj.get('assignedTeams', [])
                if isinstance(assigned_teams, list) and assigned_teams:
                    t = assigned_teams[0]
                    project_team = t.get('name') or t.get('teamName') if isinstance(t, dict) else str(t)
                if not project_team:
                    project_team = project_obj.get('team') or project_obj.get('teamName')
                if isinstance(project_team, dict):
                    project_team = project_team.get('name') or project_team.get('teamName')
                if project_team and isinstance(project_team, str) and project_team.startswith('/CxServer/'):
                    project_team = project_team.replace('/CxServer/', '', 1)

                print(f"\n{'='*80}")
                print(f"Analyzing project {i}/{len(downloaded)}: {project_name}")
                if project_team:
            ...
            print("Reliable mode: sequential per-project download")
            for i, project in enumerate(projects, 1):
                ...
                try:
                    report_file, sca_scan_date = SCA_get_report(project_name, 'csv', access_token)
                    if not report_file:
...
def analyze_report(project_name=None, report_path=None, show_all_severities=False, show_transitive=True, keep_report=False, risk_score_threshold=None, show_dev_dependencies=True, output_csv=None):
    ...
    try:
        if report_path:
            ...
            report_file, sca_scan_date = SCA_get_report(project_name, 'csv')
            if not report_file:
        ...
        # For existing report files, scan_date will be None
        scan_date_to_use = sca_scan_date if 'sca_scan_date' in locals() else None
...
def main():
    ...
    try:
        ...
        source_group.add_argument("--project_name", "--project", help="Project name in SCA portal")
        ...
        parser.add_argument("--fast", action='store_true', help="Use fast bulk parallel mode for --all_projects")
        ...
        parser.add_argument("--limit", type=int, default=None, help="Limit number of SCA and SAST projects to analyze (useful for testing)")
        ...
        if all_projects:
            ...
            if project_limit:
                print(f"Project limit: {project_limit} (testing mode)")
        elif project_name:
